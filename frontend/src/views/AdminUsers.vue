<template>
  <section class="admin">
    <header class="toolbar">
      <div>
        <h2>{{ $t('admin.userReview') }}</h2>
        <p>{{ $t('admin.userReviewDesc') }}</p>
      </div>
      <div class="actions">
        ...
        <button class="secondary" @click="setStatus('rejected2')">{{ $t('admin.rejected2') }}</button>
      </div>
    </header>
    ...
    <div class="table">
      ...
      <div class="row" v-for="user in users" :key="user.id">
        <span class="title" :data-label="$t('admin.username')">{{ user.username }}</span>
        <span :data-label="$t('admin.nickname')">{{ user.nickname || '-' }}</span>
        <span :data-label="$t('admin.role')">{{ user.role }}</span>
        <span class="status" :data-status="user.status" :data-label="$t('admin.status')">{{ statusLabel(user.status) }}</span>
        <span class="row-actions" :data-label="$t('admin.action')">
          <button v-if="user.status !== 'approved'" @click="handleApprove(user.id)">{{ $t('admin.approve') }}</button>
          <button v-if="user.status !== 'rejected'" class="secondary" @click="handleReject(user.id)">{{
...
import { approveUser, listUsers, rejectUser, type AdminUser } from "../api/admin";
...
async function fetchUsers() {
  // 按状态获取用户列表
  const data = await listUsers(status.value);
